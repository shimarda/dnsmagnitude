...
do
    ...
    if [[ "$file_time" -ge "$start_utc" && "$file_time" -le "$end_utc" ]]; then
        ...
        /usr/bin/time -v tshark \
            -r "$input_file" \
            -Y "(dns.qry.name matches "\.tsukuba\.ac\.jp$")  and (dns.flags.rcode == 0) and (dns.flags.authoritative == 0) and (ip.src == 130.158.68.25 or ip.src == 130.158.68.26 or ip.dst == 130.158.68.25 or ip.dst == 130.158.68.26)"\
            -T fields \
            -e frame.time -e ip.src -e ip.dst -e dns.qry.name -e dns.qry.type -e dns.flags.response -e vlan.id\
