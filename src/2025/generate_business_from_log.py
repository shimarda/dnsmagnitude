#!/usr/bin/env python3
"""
processed_files.log に記載された日付のうち、業務時間データが
まだ生成されていないものを検出し、生成するスクリプト

使用方法:
    # ドライラン（対象日を表示のみ）
    python3 generate_business_from_log.py --dry-run
    
    # 実行
    python3 generate_business_from_log.py
"""
...
def get_dates_from_log():
    ...
    with open(PROCESSED_LOG, 'r') as f:
        for line in f:
            # パターン: dump-202501011400.gz → 20250101
...
def get_dates_with_business_data():
    """業務時間データがある日付を取得 {date_str: [where_values]}"""
    dates = {}
...
def main():
    parser = argparse.ArgumentParser(description='processed_files.log から欠落業務時間データを生成')
    parser.add_argument('--dry-run', action='store_true', help='実行せずに対象日を表示')
    parser.add_argument('--limit', type=int, help='処理する最大件数')
    args = parser.parse_args()
    
    print("=" * 60)
    print("業務時間データ生成（processed_files.log ベース）")
    print("=" * 60)
    ...
    for date_str, where in missing:
        if generate_business_data(date_str, where):
            success += 1
        else:
            failed += 1
